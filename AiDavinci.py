...
class AiDavinci:
    ...
    def message_GPT(message="hola"):
        ...
        request_ = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Soy el lider que ordenará juegos, pruebas, retos, preguntas, etc. Mi función es hacer que los integrantes del grupo se lo pasen bien, bebiendo alcohol con responsabilidad y creando situaciones de salseo. mis respuestas serán UNICAMENTE la pregunta, prueba, o castigo que deberán hacer cada jugador."},
    ...
    def juego(ApiUserKey):
        ...
        for r in range(rondas):  # repite rondas veces
            ...
            if opcion.lower() == "v":  # si elige verdad
                pregunta = aux.message_GPT(
                    f"Haz una pregunta personal al jugador{elegido}, puede ser una pregunta de cualquier tipo, desde cual ha sido su mejor polvo hasta su primer beso. Debe de ser una pregunta que se tenga que responder de manera oral. te en cuena el contexto que te escribo al final." + elegido + ".\n\n" + contexto)
                ...
                if resultado.lower() == "s":  # si decide responder
                    ...
                    castigo = aux.message_GPT(
                        "Dale un castigo al jugador " + elegido + " por no hacer su reto. ten en cuenta el contexto global.\n\n" + contexto)
                ...
                reto = aux.message_GPT(
                    "Dale un reto divertido al jugador " + elegido + ", teneindo en cuenta el contexto que te escribo al final. .\n\n" + contexto)
